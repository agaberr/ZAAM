...
export default function UI({ setActiveTab, setIsTalking, setAudioData }) {
  ...
  const sendMessage = async (text: string) => {
    ...
    // try {
    //   const res = await fetch("http://192.168.1.7:5000/chat", {
    //     method: "POST",
    ...
    //   const { text: replyText, audio, animation } = data.messages[0];
    //   setAudioData(audio);
    ...
    const aiMessage: Message = {
      id: (Date.now() + 1).toString(),
      text: "I'm here to help you. What would you like to know?",
      sender: "ai",
      timestamp: new Date(),
    };
    setMessages((prev) => [...prev, aiMessage]);
